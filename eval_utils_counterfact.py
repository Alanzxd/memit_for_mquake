...
def calculate_metrics(
    ...
    question = prompt.replace("{}", subject)

    '''input_file = "/data/shared/Alan/memit_for_mquake/data/zsre_mend_eval.json"
    with open(input_file, 'r', encoding='utf-8') as file:
        dataset = json.load(file)
    for record in dataset:
        if record["src"] == question:
            question = record["validate_question"]
            validate_answers = record["validate_answers"]
            print(f"Matched Question: {question}")
            print(f"Validate Answers: {validate_answers}")
            break
    else:
        print("No match found.")

    # 正确答案来自 target_new
    correct_answer = validate_answers[0]
    print(correct_answer)
    if "attend" in correct_answer:
        validate_answers.append("teacher")
        validate_answers.append("professor")
        validate_answers.append("teach")
        validate_answers.append("student")
        validate_answers.append("study")
        validate_answers.append("work")
        validate_answers.append("Ph.D")
        validate_answers.append("B.S")
        validate_answers.append("M.S")
        validate_answers.append("degree")
    if "birth" in correct_answer:
        validate_answers.append("born")
        validate_answers.append("live")
        validate_answers.append("grow")
    

    answer_aliases = validate_answers
    print(answer_aliases)
    new_single_hops = record.get('new_single_hops', [])
    '''
    # Generate answer for the dynamically created question
    ...
    for single_hop in new_single_hops:
        ...
        if single_hop['answer'].lower() in generated_text.lower() or any(alias.lower() in generated_text.lower() for alias in single_hop.get('answer_alias', [])):
            success_count += 1
    '''
    multi_hop_accuracy = correct_responses
    editwise_accuracy = success_count / len(new_single_hops) if new_single_hops else 0
